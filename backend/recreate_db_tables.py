...
from app.models.course import Course, CourseCategory, Lesson, VideoStatus
...
def create_sample_courses():
    ...
    try:
        ...
        for course in db.query(Course).all():
            category_name = {
                CourseCategory.BASIC: '中医基础',
                CourseCategory.SEASONAL: '四季养生',
                CourseCategory.DIET: '药膳食疗',
                CourseCategory.MASSAGE: '推拿按摩',
                CourseCategory.HERB: '本草方剂',
                CourseCategory.DISEASE_FOCUSED: '逐病精讲',
                CourseCategory.COMPREHENSIVE: '全面学医'
            }.get(course.category, course.category.value if hasattr(course.category, 'value') else str(course.category))
            
            print(f"- {course.title} ({category_name}) - {'免费' if course.is_free else f'¥{course.price}'}")
...
def main():
    ...
    print("\n开始创建示例课程...")
    if create_sample_courses():
        print("\n数据库重建和课程创建完成！")
        print("现在可以启动后端服务测试API了")
    else:
        print("\n课程创建失败")
